...
class Tetris:
    def __init__(self,action_type = 'grouped',state_type = 'original',reward_type = 'fitness',
                 use_fitness = False,is_display = False,_screen = None):
    ...
    def check_tspin(self,piece,is_oracle = False):
        ...
        if piece_type == 'T' and self.prev_action in [3,4]:
            ...
            if n_grid >= 3:
                ...
                if piece_index == 1:
                    if 0 <= x1 <= 9  and self.grid[x1][y1] == 0 and self.grid[x2][y2] == 0:
                        tspin_type = 'mini'
    ...
    def cal_fitness_reward(self):
        ...
        return normed
    def _save(self):
        piece,grid,nextlist,piecelist = self.piece,self.grid,self.nextlist,self.piecelist
        return [piece,grid,nextlist,piecelist]
    def _load(self,x):
        self.piece,self.grid,self.nextlist,self.piecelist = x
    ...
    def get_state(self,grid,modify_last_element = False): #[grid,nextlist[0]....,held]
        ...
        if self.state_type == 'original': 
            ...
            return [grid,Y]
        elif self.state_type == 'hand_crafted':
            heights = cal_columns_height(grid)
            state = heights + [height,bumpiness,holes]
            state = np.array(state) # 13 dim
            return state,0 
    ...
    def get_all_possible_states(self): #bfs all possible move
        ...
        now_fitness = self.cal_fitness(copy.deepcopy(self.grid))
        flag = False
        if self.held != zeropieces :
            flag = True
            b = self.held
        ...
        for piece,actions in record:
            ...
            fit = self.cal_fitness(temp_grid) - now_fitness
            reward_list.append(fit)
    ...
    for x in range(10):
        ...
        for y in range(20):
            ...
            if grid[x][y] == 0 and y > highest:
                num_holes += 1
    return num_holes
def cal_columns_height(grid): # get the height of each column
    heights = []
    for x in range(10):
        height = 0
        for y in range(20):
...
if __name__ == '__main__':
    test = 'grouped'
    T = Tetris(action_type = test,use_fitness = False,is_display = False)
    state = T.reset()
    if test == 'single':
        ...
        while True:
            ...
            f = []
            import random
            actionID = random.randint(0,len(states)-1)
            state,reward,done,line_sent,line_cleared = T.step(int(actionID))
            #print(reward,done,line_sent,line_cleared)
            if done:
                print('Game Over.')
                print(idx)
                idx = 0
